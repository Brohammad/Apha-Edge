...
class AlpacaBroker(BrokerPort):
    """Alpaca Markets REST adapter. Falls back to paper simulation when credentials are missing."""
    ...
    async def submit_order(self, order: Order, market_price: Decimal) -> OrderAck:
        ...
        async with httpx.AsyncClient(timeout=20.0) as client:
            ...
            if resp.status_code >= 400:
                sim = await self._fallback.submit_order(order, market_price)
                return OrderAck(
                    broker_order_id=f"alpaca-fallback-{sim.broker_order_id}",
                    fill=sim.fill,
                )
            data = resp.json()
